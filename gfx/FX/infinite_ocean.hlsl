...
float g_fTime;
const float3 g_vSunDir = float3( -100.0, 70., 25. );
float3 GetSunDir() { return normalize( g_vSunDir ); }

const float3 g_sunColour = float3( 1.0, 0.85, 0.5 ) * 5.0;
const float3 g_skyColour = float3( 0.1, 0.5, 1.0 ) * 1.0;
const float3 k_bgSkyColourUp = g_skyColour * 4.0;
const float3 k_bgSkyColourDown = g_skyColour * 6.0;
const float3 k_envFloorColor = float3(0.3, 0.2, 0.2);
const float3 k_vFogExt = float3(0.01, 0.015, 0.015) * 3.0;
const float3 k_vFogIn = float3(1.0, 0.9, 0.8) * 0.015;
...
void initCamera(out float3 ro, out float3 rd,out float near, out float far) {
    ...
    near = 0.01;
	far = 200.0;
    float2 screenCoords = (2.0*gl_FragCoord.xy/iResolution.xy - 1.0)*aspect;
    ro = cameraPosition;
