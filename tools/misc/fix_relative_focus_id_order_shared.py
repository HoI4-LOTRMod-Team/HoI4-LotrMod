...
for focus in root_obj.value:
    if focus.id == "shared_focus":
...
def reorder_focus_objects(obj_list, focus_ord):
    ...
    focus_indices = [i for i, obj in enumerate(obj_list) if obj.id == "shared_focus"]
...
root_obj.value = l
SaveObjToFile(root_obj, filepath)